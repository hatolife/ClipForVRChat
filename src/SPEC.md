...
Windowsで `ClipForVRChat.exe` へファイルやフォルダをドラッグ&ドロップすると、渡されたパスを起動引数として処理します。

画像拡張子のファイルが1枚以上指定された場合、それらをClipForVRChatの入力画像として順番に処理します。
...
画像以外のファイルやフォルダが指定された場合は、UIを起動せず公開鍵GPG暗号化を行います。

- 画像以外のファイル: `<元パス>.gpg` を作成する。
- フォルダ: フォルダ内容をzip化したデータを暗号化し、`<フォルダパス>.zip.gpg` を作成する。

画像と画像以外のパスが混在している場合は、画像以外のパスを先に暗号化し、画像ファイルだけを入力画像として処理します。

起動引数では `.json` 設定ファイルも画像以外のファイルとして暗号化対象にします。設定ファイルをGUIで開く操作は、起動済みウィンドウへのD&Dで扱います。
...
この節は起動済みウィンドウへのD&D仕様です。`ClipForVRChat.exe` へ直接D&Dされた起動引数は、「exeへのD&D/起動引数」の仕様に従って画像処理またはGPG暗号化へ分岐します。
