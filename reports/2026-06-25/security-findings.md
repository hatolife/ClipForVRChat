...
不具合報告用データは `diagnostics/<timestamp>/` に確認用zipと暗号化済み `.zip.gpg` を作成する。確認用zipを残すこと自体は、ユーザーが送信前に内容を確認できるため有用である。一方で、zip内のテキストはパスを `%USERPROFILE%` などへ置換するが、Webhook URLや履歴内の `discordToken` は置換対象ではない。

そのため、ユーザーが誤って暗号化前zipをGitHub Issueなどへ添付すると、Discord Webhook URLや削除用token、履歴URL、ログ内容が公開され得る。平文zipはユーザー管理の確認用ファイルとして残しつつ、そこへ秘密情報を入れない設計にする必要がある。
...
確認用zipは「ユーザー確認用の安全なzip」として残す。Webhook URL、Discord token、必要に応じてmessage ID、QR URL、ファイル名をダミー値へ置換する。暗号化済み `.zip.gpg` にも同じ安全化済みzipを暗号化して入れる方針にすれば、zipとgpgの内容差分による混乱も避けられる。

### 修正例

- `sanitizeDiagnosticJSON()` を追加し、`config.json` の `discord.webhookUrl`、`autoPhoto.webhookUrl`、`screenshotAutoPost.webhookUrl` を `"<redacted>"` または `"<dummy-webhook-url>"` にする。
- `history.json` の `discordToken` を `"<redacted>"`、Discord Webhook URL相当の値を `"<dummy-webhook-url>"` にする。
- `manifest.json` に「平文zipは秘密情報を除外済み」と記録する。

### 修正後の確認方法

確認用zipと `.zip.gpg` の復号後zipの両方に、`discord.com/api/webhooks`、`discordapp.com/api/webhooks`、`discordToken` の生値が含まれないことをテストで確認する。
...
Discord投稿後、削除機能のためにWebhook ID、message ID、tokenを `history.json` へ保存している。これはDiscord上の投稿を後から削除する機能のために必要な設計判断である。一方で、ファイルは `0600` 相当で保存されるが、Windows ACLとして期待通り最小化されるかは要確認であり、診断zipにも含まれ得る。
...
短期対応として、診断データにはtokenの生値を含めず、ダミー値へ置換する。アプリ内保存については削除機能とのトレードオフがあるため、維持する場合はWindows Credential ManagerやDPAPIで暗号化する、または削除可能期間を短くしてtokenを破棄する案を検討する。

### 修正例

- 診断データ作成時は `history.json` の `discordToken` を `"<redacted>"` に置換する。
- `history.json` にはmessage IDとURLだけを保存し、削除時に現在の設定Webhook URLからtokenを再取得する方式も検討する。
- 取得不能な場合は削除不可としてUI表示する。
- token保存が必要な場合はOS保護ストレージへ移す。

### 修正後の確認方法

診断zip内に生tokenが保存されないこと。アプリ内保存方針を変更する場合は、既存履歴の移行処理でtokenが削除または保護されること。
...
Release workflowはworkflow全体で `contents: write` を持つ。これはGitHub Actionsが自動発行する `GITHUB_TOKEN` に「リポジトリのcontentsを書き換える権限」を与える設定で、Release作成には必要である。ただしworkflow全体に設定すると、ビルド・テスト・署名などの前段stepでもRelease作成やtag/asset操作に使えるtokenが存在する状態になる。また、外部Actionはメジャーバージョンタグで参照されており、コミットSHA pinningではない。
...
job単位で最小権限を設定し、可能ならRelease作成jobとビルドjobを分ける。ビルド・テストjobは `contents: read`、Release作成jobだけ `contents: write` にする。外部ActionはSHA pinningを検討し、Dependabot/Renovateで更新運用する。Release Environment protectionも検討する。

Environment protectionは、GitHubのEnvironmentに承認者や実行制限を設定し、Release jobが秘密鍵やRelease権限へ到達する前に人間の承認を要求できる仕組みである。これにより、誤ったtag pushや想定外のworkflow変更が即Release成果物作成につながるリスクを下げられる。
...
- `environment: release` を設定し、GitHub repository settings側で承認者とsecret公開範囲を制御する

### 修正後の確認方法

workflow権限がjob単位になっていること。Release jobだけがRelease用secretと `contents: write` を持つこと。Environment承認後に、従来通り成果物を作成し、署名・zip検査が通ること。
...
ACLはAccess Control Listの略で、Windowsではファイルやフォルダごとに「誰が読めるか、書けるか、削除できるか」をDACLという一覧で管理する。Unixの `0600` は「所有者だけ読み書き可」を意味するが、Goの `os.Chmod` はWindowsでは主に読み取り専用属性の制御に限られ、Unixと同じアクセス制御にはならない。つまり、Goコードで `0600` と書いていても、実際に他ユーザーから読めないかはWindowsのACLを確認する必要がある。
...
Windows実機で `icacls config.json history.json logs diagnostics` を確認する。`icacls` はWindows標準のACL表示・変更コマンドで、対象ファイルに付いているユーザー・グループ別の権限を確認できる。

### 推奨修正

Windows専用実装でDACLを明示する、または保存先をユーザープロファイル配下のアプリデータディレクトリへ移すことを検討する。DACLを明示する場合は、現在のWindowsユーザー、Administrators、SYSTEMなど必要最小限の主体だけに読み書きを許可する。
