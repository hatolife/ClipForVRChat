...
- 注意点: Windows ACLとして期待通り制限されるか要確認。Windows ACLはファイルごとのアクセス許可一覧であり、Unixの `0600` と同じ意味ではない。履歴内の絶対 `outputPath` は削除対象として解決される。
...
- 注意点: 確認用zipを残す仕様自体はユーザーが内容を確認するために必要。ただし確認用zipは安全化済みデータにし、Webhook URLやDiscord tokenをダミー値へ置換する必要がある。
