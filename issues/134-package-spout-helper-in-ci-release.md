...
- Spout2 CMakeは `SpoutLibrary` DLLターゲットを公開し、内部で `Spout_static` へリンクする。`spout-capture.exe` は `SpoutLibrary.dll` と同じフォルダで実行できるようにする。
...
- Release zip展開先に対し、`ClipForVRChat.exe`、`spout-capture.exe`、`SpoutLibrary.dll`、`README.md`、`LICENSE`、`Spout2-LICENSE.txt`、`Release-signing-public-key.url` が存在するか確認する。
...
- `tools/spout-capture/CMakeLists.txt` は `SpoutLibrary_static` をリンクしているが、上流 Spout2 2.007.015 の CMake は `SPOUTSDK/SpoutLibrary/CMakeLists.txt` で `add_library(SpoutLibrary SHARED ...)` を定義している。
- そのため CI の `cmake --build ... --target spout-capture` は `SpoutLibrary_static.lib` を探しにいき、生成されないファイル名を要求して `LNK1181` になっている。
- 最小修正は `SpoutLibrary_static` 参照を上流の実ターゲット名に合わせること。
