...
- [ ] 自動処理ONかつDiscord投稿ONの場合、監視フォルダと有効Webhookを保存前確認に含める。
- [ ] Webhookが設定済みの場合でも、外部設定由来または非表示フィールド由来の監視フォルダを確認対象にする。
- [ ] 自動処理の監視フォルダを設定画面から確認・変更できる。
- [ ] 広すぎる監視フォルダや危険な既知フォルダを安全側に扱う方針を実装または明文化する。
- [ ] 回帰テストを追加する。
