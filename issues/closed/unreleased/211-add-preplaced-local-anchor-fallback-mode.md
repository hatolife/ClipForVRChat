...
- フォールバックモード自動ON設定がONの場合のみ、AvatarBeaconからのbasis受信が30秒以上ない状態をフォールバック候補として扱う。
- フォールバックモード自動OFF設定がONの場合のみ、AvatarBeaconのbasis受信が戻ったら通常モードへ戻す。
- 既定はOFFで、従来の構図制御は維持する。

## 対応内容

- `autoCapture.capture.preplacedLocalAnchor` を追加した。2026-07-07に #235 で既定OFFへ修正し、自動ON/OFF制御は別設定へ分離した。
...
- 2026-07-07: #222の「自動切り替え廃止」「フォールバックモード既定ON」はユーザー指示の解釈誤りだったため、#235で自動ON/OFF制御を別設定として追加し、既定OFFへ修正した。
