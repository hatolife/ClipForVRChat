...
- 2026-07-07: a43 helperを使ったWindows版一時ハーネスで `AutoCaptureRunner.RunOnce` 相当経路を実行し、初回sender 0件からStreaming OFF/ON回復、再確認sender 1件、`spout capture success` まで到達することを実機確認した。診断ログに回復前後のsender数と各OSC送信結果が記録された。
