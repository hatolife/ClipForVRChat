...
v0.1.8-a12時点のStream方式は、ffmpegでデスクトップやVRChatウィンドウ周辺を取得しており、ユーザーが求める「VRChat Stream Camera映像そのもの」の保存になっていない。
...
- 2026-07-01補足: `spout-capture.exe` と `SpoutLibrary.dll` は「ClipForVRChat本体ではできない」ためではなく、Spout/DirectX/OpenGL/Windowsネイティブ依存を本体プロセスから隔離するために分離している。既存の `ClipForVRChat.exe` へ統合すること自体は将来的には可能だが、CGO/C++連携、DLLロード、GPUリソース、クラッシュ時の影響範囲、CI/Release検証の複雑さが増える。v0.1.8では、Wails本体はOSC制御、設定、履歴、sidecar、Discord連携を担当し、Spout受信とPNG保存は小さいWindows専用ヘルパーへ任せる方針を採った。
- 2026-07-01補足: `SpoutLibrary.dll` は `spout-capture.exe` がリンクするSpout2 SDKの実行時DLLであり、VRChat Stream Cameraのsender列挙とフレーム受信に必要。`spout-capture.exe` だけ、または `SpoutLibrary.dll` だけではStream方式は動作しない。
