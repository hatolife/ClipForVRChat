# rc36でStream Camera起動済みでもSpout取得が透明フレームになる

## 問題

`v0.1.8-rc36` のテスト撮影で、VRChat内のStream Cameraを起動しSpoutもONにしているにもかかわらず、Stream方式のSpout取得が次のエラーで失敗する。
...
- `/mnt/c/Users/user/Downloads/ClipForVRChat-v0.1.8-rc36-windows-amd64/logs/2026-07-05.log`
...
- [x] rc36ログを確認し、sender検出済み・receive成功・frame番号不変・全透明で失敗していることを記録する。
...
- 2026-07-05: rc36ログでは `VRCSender1` を検出し、`receiveAttempts=265` / `receiveSuccesses=264` まで進んでいる。一方で `frame=0` / `firstFrame=0` / `lastReceivedFrame=0` / `transparentRatio=1.0` で、sender frame番号が進まない全透明受信で失敗していた。
