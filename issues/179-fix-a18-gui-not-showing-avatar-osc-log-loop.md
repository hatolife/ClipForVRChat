# a18でGUIが表示されずavatar_oscエラーが大量出力される

## 問題

`v0.1.8-a18` の単一exe版、separated版のどちらでも起動してもGUIが表示されない。ユーザー提供ログでは、起動直後から `avatar osc basis resolve error: status="partial" err="partial avatar OSC basis sample: missing coord/x"` などが大量に連続出力され、短時間でログが肥大化している。
...
- `v0.1.8-a19` separated版でもGUIが表示されない。
...
- `v0.1.8-a20` separated版で、簡易起動画面に `フロントエンドエラー: Uncaught ReferenceError: avatar is not defined at http://wails.localhost/assets/index-DvTLSiJp.js:397:68` が表示された。
