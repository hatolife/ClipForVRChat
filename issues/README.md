...
作業が完了したチケットはこの一覧から外し、チケットファイルを `issues/closed/vX.Y.Z/` などの分類フォルダへ移動して `issues/closed/README.md` に状態 `完了` の行として追加します。
...
| 296 | [Codex Security findings全件対処に向けた軽量調査レポートを作成する](296-create-security-findings-remediation-report.md) | 要対応 | 未定 | 新CSV全44件を底本に、指定ディレクトリ配下へ全件対処の管理レポートを作成する。 |
| 297 | [Codex Security findingsレポートを分類して追記する](297-classify-security-findings-remediation-report.md) | 要対応 | 未定 | 重大仕様変更、追加情報要否、即時修正対象を分類し、管理レポートへ追記する。 |
| 298 | [追加情報不要のCodex Security findings修正をサブエージェントで監督する](298-supervise-subagents-for-actionable-security-findings.md) | 要対応 | 未定 | 仕様変更・追加情報待ちを除いたfindingを作業単位へ分け、サブエージェントに委任して監督する。 |
| 299 | [Codex Security findings修正作業の報告をレポートへ追記する](299-append-security-findings-remediation-work-report.md) | 要対応 | 未定 | 即時修正対象の対応完了後、対応内容、テスト、保留理由、コミットを管理レポートへ追記する。 |
