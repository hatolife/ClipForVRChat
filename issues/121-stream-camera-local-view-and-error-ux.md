...
v0.1.8-a8の初期Poseはワールド原点基準で撮影され、ユーザーが想定するプレーヤー中心の正面、背面、斜め視点にならない。
...
- a9実機確認で、Stream方式でも `/usercamera/Mode` にPhoto Camera相当の `1` を送っていたため、Stream方式では `2` を送るよう修正した。
...
- このissueはv0.1.8自動撮影の統合確認issueとして扱う。実コードの主な修正は [#129](129-add-spout-capture-helper.md) から [#135](closed/v0.1.8/135-add-spout-stream-camera-verification-guide.md)、[#138](closed/v0.1.8/138-define-player-local-coordinate-spec.md)、[#140](closed/v0.1.8/140-implement-player-local-pose-transform.md)、[#141](141-integrate-player-local-coordinate-ui.md)、[#163](closed/v0.1.8/163-show-autocapture-test-results-in-settings.md) で行う。
