...
- 2026-07-01: 埋め込み側の実装に合わせて `app_version`、`user_count`、`users_truncated`、`resolved_pose`、`metadata_warnings` を含む schema 拡張を進める。
- 2026-07-01: サイズ上限超過時はユーザー配列を切り詰め、画像保存全体は失敗扱いにせず warning として sidecar / log へ残す方針で実装する。
