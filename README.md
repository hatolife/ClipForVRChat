...
画像URLの履歴は設定ファイルと同じ場所の `history.json` に保存されます。
...
クリアしてもURL履歴は削除されず、履歴にはクリア済みとして残ります。

クリアボタンを5秒長押しすると画像履歴画面を開けます。履歴画面では、削除済みやクリア済みを含めた過去URLの画像を確認できます。

履歴画面ではCtrlクリックで複数選択、Shiftクリックで範囲選択できます。選択した履歴はDiscord上の投稿だけを削除できます。履歴や元画像ファイルは削除されません。

「削除済みURLの履歴を削除」は、URLが既に参照できない履歴だけを `history.json` から削除します。
