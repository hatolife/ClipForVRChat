...
設定画面の「撮影処理」カテゴリでは、VRChat User CameraをOSCで操作し、指定した構図を順番に撮影できます。

自動撮影は初期状態では無効です。使う場合は「機能 ON/OFF」カテゴリの「自動定期撮影」をONにします。同じ場所で撮影間隔、初回待機時間、開始時撮影を設定できます。開始時撮影は既定ONですが、AvatarBeacon basisが使える状態、またはフォールバックモードが使える状態になるまで待ってから実行します。
...
設定画面は、画面右上の「設定」ボタンから開けます。左側の縦ナビゲーションでカテゴリを選ぶと、右側に設定項目が表示されます。

### 機能 ON/OFF

- VRChat写真自動処理: VRChat上で撮影されたときに処理します。
- スクリーンショット自動処理: Win + Shift + Sでスクリーンショットが撮られたときに処理します。
- 自動定期撮影: VRChatカメラの定期撮影をON/OFFし、撮影間隔、初回待機時間、開始時撮影を設定します。
...
- 撮影と出力: Stream/Photo方式、Spout helper、sender、自動選択、各操作の待機時間、録画デバッグ、保存先、保存形式、ファイル名、sidecar JSON、画像埋め込みメタデータを設定できます。
- Camera状態復元: 撮影後にUser CameraのMode、Pose、Streaming、Zoom、Exposure、表示対象mask類などをできるだけ戻します。
- 撮影終了時: Camera LockとFlyingをOFFにします。待機カメラ位置への移動は任意で、既定OFFです。
...
自動撮影の `player_local` は AvatarBeacon の `avatar_osc` basis を既定にし、専用アバターギミックがない場合だけ `manual` basis またはフォールバックモードに切り替えて使います。受信状態や最終受信時刻、position/yaw は設定画面の撮影処理カテゴリとOSCカテゴリで確認できます。
...
保存先、出力形式、Discord投稿などを変更できます。設定項目は左側の縦ナビゲーションでカテゴリごとに切り替えて表示します。

出力先フォルダは、入力欄へ貼り付けても、「選択」ボタンからフォルダを選んでも設定できます。

未保存変更がある場合は、変更されたカテゴリや項目が強調されます。設定画面を閉じる、別画面へ移動する、またはウィンドウを閉じる前に、保存、破棄、キャンセルを選べます。
