...
PGP署名を確認する場合は、zipを展開して `ClipForVRChat.exe` を取り出します。GitHub Releasesに別添付されている同じバージョンの `.exe.asc` をexeと同じフォルダに保存してください。公開鍵は `release-signing@hato.life` の鍵を使用し、取り込んだ鍵のfingerprintが `BE40 AA8D 082F 493F 613B C072 21DC 3486 1B40 E77D` と一致することを、このREADMEや公式配布ページなどRelease assetとは別の信頼経路で確認してください。同じReleaseに同梱されたURLや公開鍵だけでは、公開鍵自体の真正性は確認できません。信頼済みfingerprintの公開鍵で `gpg --verify` が成功した場合に限り、exeと署名の組み合わせがその鍵で署名されたものだと判断できます。
...
Stream方式はVRChat Stream CameraのSpout映像を同梱の `spout-capture.exe` で直接受信して保存します。配布zipでは `spout-capture.exe` と実行に必要な `SpoutLibrary.dll` が `ClipForVRChat.exe` と同じフォルダに入ります。通常はユーザーが直接起動せず、ClipForVRChatが自動撮影時に呼び出します。Photo方式はVRChat標準写真を使うフォールバックで、VRChat側のシャッター音が発生します。

`spout-capture.exe` と `SpoutLibrary.dll` は、VRChat Stream Cameraの実体であるSpout senderから映像フレームを受け取るために必要です。ClipForVRChat本体へ統合すること自体は将来的に可能ですが、Spout/DirectX/OpenGL/DLLロードなどのWindowsネイティブ処理を本体プロセスに混ぜると、GPUやDLLまわりの失敗がアプリ全体のクラッシュにつながりやすくなります。そのためv0.1.8では、Spout受信だけを小さい別プロセスへ分離し、本体はOSC制御、設定、保存後処理、履歴、Discord投稿を担当します。
...
配布物を確認するときは、公式Releaseまたは公式配布元からzipを取得し、Releaseに掲載されたsha256と一致すること、zip内に `ClipForVRChat.exe`、`spout-capture.exe`、`SpoutLibrary.dll`、`Spout2-LICENSE.txt` がそろっていることを確認してください。`spout-capture.exe` だけ、または `SpoutLibrary.dll` だけではStream方式は動作しません。

Stream方式で `Spout helperが見つかりません`、`Spout helperは見つかりましたが実行確認に失敗しました` と表示される場合は、配布zipを展開したフォルダに `spout-capture.exe` と `SpoutLibrary.dll` が両方残っているか確認してください。どちらかを削除、リネーム、別フォルダへ移動するとStream Camera映像を取得できません。
