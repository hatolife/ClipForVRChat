...
- 出力形式: PNGまたはJPGを選べます。
- JPEG品質: JPG出力時だけ使います。
- 履歴削除時にoutputも削除: Discord削除済み履歴を削除するとき、ローカル保存したoutput画像も削除します。
...
Discord投稿がOFFの場合、投稿URLの自動コピー、Webhook、自動処理の関連設定はグレーアウトします。ローカル保存がOFFの場合、ローカル保存に関係する設定はグレーアウトします。
