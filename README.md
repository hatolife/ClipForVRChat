...
- QRコードURL検出: 画像内のQRコードを読み取り、URLをDiscord本文と結果画面に表示します。
- 出力形式: PNGまたはJPGを選べます。
- JPEG品質: JPG出力時だけ使います。
...
- 自動投稿: VRChatの写真フォルダに新しい画像が保存されたとき、自動で処理します。
- スキャン間隔: 自動投稿でフォルダを確認する間隔です。初期値は2秒です。
- 写真フォルダ: VRChatが写真を保存するフォルダです。通常は `Pictures\VRChat` です。

### スクリーンショット自動投稿

- 自動投稿: Screenshotsフォルダに新しい画像が保存されたとき、自動で処理します。初期値はOFFです。
- スキャン間隔: スクリーンショット自動投稿でフォルダを確認する間隔です。初期値は2秒です。
- Screenshotsフォルダ: Windowsのスクリーンショット保存先です。通常は `Pictures\Screenshots` です。

Discord投稿がOFFの場合、1枚時のURL自動コピー、Webhook、自動投稿の関連設定はグレーアウトします。自動投稿がOFFの場合も、スキャン間隔や監視フォルダなどの詳細設定はグレーアウトします。
